...
Raphael.fn.freeTransform = function(subject, options, callback) {
    ...
    if ( ft.opts.showBBox ) {
        var curBox = paper.rect(0,0,0,0);
        
        ft.drawBBox = function() {
        ...
        ft.hideBBox = function() { 
            curBox.hide();
        };
        
        ft.showBBox = function() {
            ft.drawBBox();
            curBox.show();
        };
    }
    ...
    ft.updateHandles = function(thing) {
        ...
        if ( ft.opts.rotate || ft.opts.scale ) {
            ft.axes.map(function(axis) {
                ...
                ft.handles[axis].line.attr({ path: [
                    'M', thing.center.x, thing.center.y,
            ...
            item.drag(function(dx, dy) {
                ...
                ft.items.map(function(item) {
                    item.transform([
                        ...
                        'T', ( dx + ft.o.translate.x - snap.x ), 
                             ( dy + ft.o.translate.y - snap.y )])
                });
